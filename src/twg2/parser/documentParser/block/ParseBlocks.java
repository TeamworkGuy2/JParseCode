...
/** Static methods to parse block of text marked by start and end characters or strings.
 * For example the input string:<br>
 * <pre>function parser.test(t) {
 ...
 * }</pre>
 * 
 * Would return 4 block representing the strings:<br>
...
public final class ParseBlocks {
	...
	/** Identify the block in the {@code input} string.
	 ...
	 * @return a {@link TextBlock} with nested block describing each of the block in {@code input}
	 */
	...
	/** Identify the block in the {@code input} string.
	 * @param block the destination root {@link IntermediateBlock} to store parsed
	 ...
	 * @param dst the destination to store pairs of start and end block indices in.
	 * The indices are zero based character indices into the {@code input} string.
	 * A {@link TextBlock} with nested block describing each of the block in {@code input}
	 */
	public static final void splitIntoBlocks(IntermediateBlock blocks, String input, String startStr, String endStr,
		...
		while(endIndex > -1) {
			...
			if(blockDepth.hasParentBlock()) {
				...
				throw new IllegalStateException("mismatching number of opening/closing block");
			}
			level--;
			loopI++;
		}
